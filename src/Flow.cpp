...
Flow::Flow(std::string flow_type) {
    if (flow_type == "Couette2D")
        ...
        throw std::invalid_argument("Flow type ["+flow_type+"] not recognized");
}
...
void Flow::init_flow_couette_2d() {
    ...
    _domain = std::make_unique<Domain>
        (_lbmodel.get(), 100, 1, 25, 0.5f, 1.0f, init_flow_vel, ext_force);
    BoundaryType type = {
        {"east", "periodic"},
        {"west", "periodic"},
        {"up", "noslip"},
...
void Flow::init_flow_poiseuille_2d() {
    _lbmodel = std::make_unique<LBModel>("D2Q9");
    std::array<float, 3> init_flow_vel = {0.0, 0.0, 0.0};
    std::array<float, 3> ext_force = {1.0e-5, 0.0, 0.0};
    _domain = std::make_unique<Domain>
        (_lbmodel.get(), 256, 1, 48, 0.1f, 1.0f, init_flow_vel, ext_force);
    BoundaryType type = {
        {"east", "periodic"},
        {"west", "periodic"},
        {"up", "noslip"},
...
void Flow::init_flow_poiseuille_3d() {
    ...
    std::array<float, 3> ext_force = {1.0e-5, 0.0, 0.0};
    _domain = std::make_unique<Domain>
        (_lbmodel.get(), 256, 256, 48, 0.1f, 1.0f, init_flow_vel, ext_force);
    BoundaryType type = {
        {"east", "periodic"},
        {"west", "periodic"},
        {"north", "periodic"},
