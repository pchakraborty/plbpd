    ...
    for (auto zl = e.zbegin; zl < e.zend; ++zl) {
        for (auto yl = e.ybegin; yl < e.yend; ++yl) {
            for (auto xl = e.xbegin; xl < e.xend; ++xl) {
                ...
                auto ulocal = simdata.u->get(zl, yl, xl, 0);
                for (auto i = 0; i < 3; ++i)
                    ueq[i] = ulocal[i] + ext_force[i]*_tau*rhoinv;
    ...
    tbb::parallel_for
        (uint32_t(e.zbegin), e.zend,
         [this, &simdata, &e, kdim] (size_t zl) {
            ...
            for (auto k = 0; k < kdim; ++k)
                cu[k] = 0.0f;  // scratch space to compute dot(ck, ueq)
            for (auto yl = e.ybegin; yl < e.yend; ++yl) {
                for (auto xl = e.xbegin; xl < e.xend; ++xl) {
                   auto zyx = simdata.rho->sub2ind(zl, yl, xl);
#if defined(AVX2)
                   _collision_kernel_avx2(zyx, kdim, c, w, extf, n, rho, u, cu);
#else
                   _collision_kernel(zyx, kdim, c, w, extf, n, rho, u, cu);
#endif
                }
            }
            _mm_free(cu);
...
inline void CollisionSRT::_collision_kernel_avx2(
    ...
    for (auto k = 0; k < (kdim/fpsr)*fpsr; k+=fpsr) {  // loop unrolling
        ...
        // 3*cu(k) + 1
        auto _neq = _mm256_fmadd_ps(_three, _cu, _one);
        // 4.5*cusq(k) + 3*cu(k) + 1
        _neq = _mm256_fmadd_ps(_fourPointFive, _cusq, _neq);
        // -1.5*usq + 4.5*cusq(k) + 3*cu(k) + 1
        _neq = _mm256_add_ps(_neq, _kusq);
